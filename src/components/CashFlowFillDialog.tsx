...
export function CashFlowFillDialog({ label, sourceYear, lastYear, visibleLastYear, initialTarget, value, values, stale, onClose, onConfirm }: Props) {
  ...
  return <dialog ref={ref} className="cashflow-fill-dialog" aria-labelledby="cashflow-fill-title" aria-describedby="cashflow-fill-description" onCancel={event => { event.preventDefault(); onClose() }}>
    ...
    <p id="cashflow-fill-description">将「{label}」{sourceYear} 年的 {value.toLocaleString('zh-CN')} 元复制到下方，仅修改本列。</p>
