...
vi.mock('./localDb', () => ({ getLocalWorkspace: () => 'self:test', getSyncMetadata: vi.fn(async () => undefined), acknowledgeCustomer: vi.fn(async () => {}) }))
...
describe('intake upload session acknowledgement', () => {
  it.each(['logout', 'switch'] as const)('does not restore an old invitation after %s', async mode => {
    let finish!: (value: { error: null; data: { status: string; revision: number } }) => void
    ...
    finish({ error: null, data: { status: 'accepted', revision: 1 } }); await request
