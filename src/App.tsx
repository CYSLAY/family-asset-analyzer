...
export function App() {
  ...
  if (!workspaceMode) return <AccessGate onAdminAllowed={(username) => { setAccessUser(username); setWorkspaceMode('admin'); setWorkspaceSync('idle'); setView('intake') }} onStartSelfService={() => { setWorkspaceMode('self_service'); setPublicReady(false); setView('intake') }} />
  ...
  return <div className={selfService ? 'app-shell self-service-shell' : 'app-shell'}>
    ...
    <main className="main-content">
      <header className="topbar">
        <div><span className="topbar-title">{view === 'intake' ? selfService ? '资料填写' : '信息录入' : view === 'customers' ? '客户档案' : selfService ? '我的分析报告' : '财务分析报告'}</span>{selectedCustomer ? <span className="topbar-customer">{selectedCustomer.householdName}</span> : null}</div>
